 ...
 */
 class ListsSuite {
  ...
  @Test def `sum of empty list (10pts)`: Unit = {
    assert(sum(List()) == 0)
  }

  @Test def `sum of repeated numbers(10pts)`: Unit = {
  ...
  @Test def `max of empty list throws (10pts)`: Unit = {
    try {
      max(List())
    } catch {
  ...
  @Test def `max of same numbers (10pts)`: Unit = {
    assert(max(List(1, 1, 1)) == 1)
  }

  @Test def `max of negative numbers (10pts)`: Unit = {
    assert(max(List(-1, 1, 1)) == 1)
  }
