...
public final class OneDimAveragingPhaser {
    ...
    public static void runParallelFuzzyBarrier(final int iterations,
    		...
    		for (int ii = 0; ii < tasks; ii++) {
                ...
                for (int iter = 0; iter < iterations; iter++) {
                    final int left = i * (n / tasks) + 1;
                    threadPrivateMyNew[left] = 0.5 * (threadPrivateMyVal[left - 1] + threadPrivateMyVal[left + 1]);
                    
                    final int right = (i + 1) * (n / tasks);
                    threadPrivateMyNew[right] = 0.5 * (threadPrivateMyVal[right - 1] + threadPrivateMyVal[right + 1]);
                    
                    int currentPhase = ph.arrive();
                    
                    for (int j = left + 1; j <= right - 1; j++) {
                        threadPrivateMyNew[j] = 0.5 * (threadPrivateMyVal[j - 1]
                            + threadPrivateMyVal[j + 1]);
                    }
