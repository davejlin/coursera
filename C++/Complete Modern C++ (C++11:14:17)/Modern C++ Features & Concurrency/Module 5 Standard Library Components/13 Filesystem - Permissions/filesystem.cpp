...
void Permissions(string_view file) {
	fs::path file_to_modify{file} ;
	if(!fs::exists(file_to_modify)) {
		cout << "Path does not exist = >" << file_to_modify.string() << endl ;
		return ;
	}
	auto perm = fs::status(file_to_modify).permissions() ;
	demo_perms(perm) ;

	cout << "Changing permissions - adding exec to test.txt\n" ;
	fs::permissions(file_to_modify,fs::perms::owner_exec, fs::perm_options::add) ;
	
	perm = fs::status(file_to_modify).permissions() ;
	demo_perms(perm) ;

	cout << "Changing permissions - removing exec from test.txt\n" ;
	fs::permissions(file_to_modify,fs::perms::owner_exec, fs::perm_options::remove) ;
...
int main() {
	...
	Permissions("/Users/davilin/Documents/Projects/coursera/C++/Complete Modern C++ (C++11:14:17)/Modern C++ Features & Concurrency/Module 5 Standard Library Components/13 Filesystem - Permissions/test.txt");
}